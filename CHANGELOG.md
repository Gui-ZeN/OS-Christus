...
- **Aviso de e-mail bloqueado/rejeitado (bounce)** (`eaa6e1f`, `d95ef79`): quando o
  provedor de destino rejeita um e-mail enviado, o Gmail devolve um NDR ("Message
  blocked") que antes era descartado. Agora o sistema detecta o bounce, resolve a OS
  (pelo `X-OS-Ticket-ID` embutido) e registra um aviso no histórico + notificação
  Admin/Gestor. Um único NDR que liste **vários** destinatários bloqueados vira **um**
  aviso com todos eles (não 1 por nome).
