...
- **Apelido por-sede no catálogo** (`site.aliases[]`): o matcher passa a casar por apelidos gravados no próprio doc da sede, com precedência sobre o mapa hardcoded — ou seja, **apelido novo sem precisar de deploy** (a dor recorrente: CESIU, PRÉ SUL, JV, PQL 2/3 exigiram deploy). Aditivo e seguro (nenhuma sede tem o campo hoje). Falta só uma UI em Configurações para editar; por ora dá pra popular direto no banco.
