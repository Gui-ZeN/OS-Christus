...
/**
 * A regra mora em `api/_lib/currency.js`, importada — ela existia em QUATRO lugares
 * com três comportamentos, e a mesma entrada virava um número aqui e outro no
 * servidor. A assinatura daqui não muda: quem chama continua recebendo `number`.
 */
export function parseCurrency(value: string | null | undefined): number {
  return parseCurrencyOrNull(value) ?? 0;
}
